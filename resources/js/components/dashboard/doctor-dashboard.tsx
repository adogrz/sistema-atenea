...
export function DoctorDashboard({ data }: DoctorDashboardProps) {
    ...
    const handleViewRecord = (recordId: number) => {
        router.get(route('clinical-records.medical-records.show', recordId));
    };
    ...
    return (
        <div className="space-y-6">
            {/* Encabezado */}
            <div>
                <h2 className="text-2xl font-bold tracking-tight">Mi Dashboard Médico</h2>
                <p className="text-muted-foreground">Resumen de tus estudiantes asignados y consultas</p>
            </div>
                                ...
                                >
                                    <div className="flex-1">
                                        ...
                                        <div className="mt-1 flex items-center gap-4 text-xs text-muted-foreground">
                                            ...
                                            {assignment.last_consultation && (
                                                <>
                                                    <span>•</span>
                                                    <span>Última consulta: {assignment.last_consultation}</span>
                                                </>
                                            )}
                                        </div>
                                    </div>
