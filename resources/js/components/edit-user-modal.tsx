...
interface EditUserModalProps {
    open: boolean;
    onClose: () => void;
    onSave: (updates: { name: string; email: string; status: string; role_name: string[]; sede_name: string }) => void;
    user: User | null;
    roles: Role[];
    sedes: Sede[];
...
export default function EditUserModal({ open, onClose, onSave, user, roles, sedes, errors = {} }: EditUserModalProps) {
    ...
    useEffect(() => {
        if (user) {
            ...
            setSelectedRoles(Array.isArray(user.roles) ? user.roles.map((role) => role.name) : user.roles ? user.roles.split(',') : []);
        }
    }, [user]);
        ...
        <>
            <Dialog open={open} onOpenChange={onClose}>
                <DialogContent className="sm:max-w-lg">
                    ...
                    <div className="space-y-2">
                        <Label>Roles</Label>
                        <div className="flex max-h-40 flex-col gap-1 overflow-y-auto rounded border px-2 py-1">
                            {roles.map((rol) => (
                    ...
                    <DialogFooter className="pt-6">
                        ...
                        <Button onClick={() => setShowConfirm(true)} disabled={!name || !email || !selectedRoles || !selectedSede || !status}>
                            Guardar
                        </Button>
                    </DialogFooter>
                </DialogContent>
